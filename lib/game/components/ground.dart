...
class Ground extends PositionComponent {
  ...
  void _rebuildCache(double t, double w, double groundTop) {
    ...
    final topColor = _sunsetColor(const Color(0xFF4A90D9),
        const Color(0xFFCC4422), const Color(0xFF0A0A2E), t);
    final midColor = _sunsetColor(const Color(0xFF87CEEB),
        const Color(0xFFE8733A), const Color(0xFF1A1A4E), t);
    final botColor = _sunsetColor(const Color(0xFFB0E0E6),
        const Color(0xFFF5A662), const Color(0xFF2A2A5E), t);

    _cachedSkyPaint = Paint()
      ..shader = Gradient.linear(
        const Offset(0, 0),
        Offset(0, RatitaGame.viewportH),
        [topColor, midColor, botColor],
        [0.0, 0.4, 1.0],
      );

    _cachedMtnPaint = Paint()
      ..color = _lerpColor(const Color(0xFF6B8E5A), const Color(0xFF1A3A1A), t);
    _cachedHillPaint = Paint()
      ..color = _lerpColor(const Color(0xFF7CAA5E), const Color(0xFF2A5A2A), t);
    _cachedGroundPaint = Paint()
      ..color = _lerpColor(const Color(0xFF8B7355), const Color(0xFF3A2A15), t);
    _cachedLinePaint = Paint()
      ..color = _lerpColor(const Color(0xFF6B5B3E), const Color(0xFF2A1A0E), t);
    _cachedGrassPaint = Paint()
  ...
  void render(Canvas canvas) {
    ...
    canvas.drawRect(
        Rect.fromLTWH(0, 0, w, RatitaGame.viewportH), _cachedSkyPaint!);

    if (t > 0.3) {
      final moonAlpha = ((t - 0.3) / 0.7).clamp(0.0, 1.0);
      final moonPaint = Paint()
        ..color = Color.fromARGB((moonAlpha * 255).round(), 255, 255, 220);
      canvas.drawCircle(Offset(w - 120, 65), 30, moonPaint);
      canvas.drawCircle(
          Offset(w - 112, 60), 26, Paint()..color = _cachedSkyPaint!.color);
    }
    ...
    if (t > 0.2) {
      ...
      for (int i = 0; i < _starX.length; i++) {
        ...
        _cachedStarPaint!.color =
            Color.fromARGB((starAlpha * twinkle * 255).round(), 255, 255, 200);
        canvas.drawCircle(
            Offset(_starX[i], _starY[i]), _starSize[i], _cachedStarPaint!);
      }
    }

    final ct = _cloudTime;

    _drawCloud(_cloud0, canvas, 50 + sin(ct * 0.3) * 3, 30 + cos(ct * 0.25) * 2,
        120, 50);
    _drawCloud(_cloud1, canvas, 250 + sin(ct * 0.22 + 1) * 4,
        55 + cos(ct * 0.18 + 2) * 3, 70, 35);
    _drawCloud(_cloud2, canvas, 500 + sin(ct * 0.28 + 2) * 3,
        20 + cos(ct * 0.2 + 1) * 2, 100, 45);
    _drawCloud(_cloud1, canvas, 100 + sin(ct * 0.15 + 3) * 4,
        80 + cos(ct * 0.22 + 3) * 3, 80, 38);
    _drawCloud(_cloudLong, canvas, 350 + sin(ct * 0.2 + 4) * 3,
        100 + cos(ct * 0.15 + 4) * 2, 160, 45);
    _drawCloud(_cloud0, canvas, 650 + sin(ct * 0.25 + 5) * 4,
        65 + cos(ct * 0.3 + 5) * 3, 75, 36);
    _drawCloud(_cloud2, canvas, 800 + sin(ct * 0.18 + 6) * 3,
        130 + cos(ct * 0.2 + 6) * 2, 100, 45);
    _drawCloud(_cloudLong, canvas, 150 + sin(ct * 0.22 + 7) * 4,
        150 + cos(ct * 0.16 + 7) * 3, 180, 50);
    _drawCloud(_cloud1, canvas, 700 + sin(ct * 0.3 + 8) * 3,
        125 + cos(ct * 0.25 + 8) * 2, 70, 33);

    if (_showBolt) {
      final bolt = _random.nextBool() ? _bolt0 : _bolt1;
      if (bolt != null) {
        bolt.render(canvas,
            position: Vector2(w * 0.3 + _random.nextDouble() * w * 0.4, 20),
            size: Vector2(80, 150));
      }
    }
    ...
    canvas.drawRect(
        Rect.fromLTWH(0, groundTop, w, RatitaGame.viewportH - groundTop),
        _cachedGroundPaint!);
    ...
    for (double i = 0; i < w; i += 32) {
      canvas.drawRect(
          Rect.fromLTWH(i, groundTop + 10, 12, 3), _cachedDashPaint!);
    }

    if (_isRaining) {
      _cachedRainPaint ??= Paint()
        ..color = const Color.fromARGB(120, 180, 200, 255);
      for (final drop in _rainDrops) {
        canvas.drawLine(
  ...
  void _drawCloud(
      Sprite? sprite, Canvas canvas, double x, double y, double w, double h) {
    if (sprite != null) {
  ...
  void _drawTree(
      Sprite? sprite, Canvas canvas, double x, double groundTop, double h) {
    if (sprite != null) {
      sprite.render(canvas,
          position: Vector2(x, groundTop - h), size: Vector2(h * 0.7, h));
    }
  }
}
